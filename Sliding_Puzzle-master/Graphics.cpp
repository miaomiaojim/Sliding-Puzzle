...
bool Graphics::setup() {
    ...
    cat = IMG_LoadTexture(renderer, "assets/cat2.jpg");
    if (cat == nullptr) {
    ...
    for (int i = 0; i < t.size(); ++i) {
        ...
        if (drawnumber){
            if (t[i].tileType() != Tile::type::invisible) {
                ...
                tilenumber = renderText(num, "assets/Brush Script.ttf", fontcolour, 25);
...
void Graphics::updateClicks(const int& n) {
    std::string message = "Clicks: " + std::to_string(n);
    clicks = renderText(message, "assets/Brush Script.ttf", fontcolour, 25);
    int W; int H;
    ...
    renderTexture(clicks, (0.76*(clickBanner.x+clickBanner.w) - W/2), (0.8*(clickBanner.y+clickBanner.h) - H), nullptr);
}

void Graphics::winMessage(const bool& win, const int& clicks, const std::string& message) {
    if (win == true && clicks > 0){
        wintext = renderText(message, "assets/Brush Script.ttf", fontcolour, 28);
        int W; int H;
...
void Graphics::menuInstruction(const std::string& message) {
    ...
    menuinstr = renderText(message, "assets/Brush Script.ttf", dark, 25);
    int W; int H;
...
void Graphics::menuText(std::vector<Tile> tiles, const bool& catmode) {
    ...
    for (int i = 0; i < 6; ++i){
        ...
        menutext = renderText(message, "assets/Brush Script.ttf", fontcolour, 28);
    ...
    SDL_Color tilecolour {67, 110, 138};
    menutext = renderText("Cat Mode", "assets/Brush Script.ttf", tilecolour, 28);
    SDL_QueryTexture(menutext, NULL, NULL, &W, &H);
    ...
    if (catmode){
        SDL_Color tilecolour {205, 0, 205};
        menutext = renderText("Cat Mode", "assets/Brush Script.ttf", tilecolour, 28);
        SDL_QueryTexture(menutext, NULL, NULL, &W, &H);
        renderTexture(menutext, (tiles[6].position().x + 0.5*tiles[6].position().w - 0.5*W + 2),
                                (tiles[6].position().y + 0.5*tiles[6].position().h - 0.5*H + 2), nullptr);
    ...
    menutext = renderText("START", "assets/Brush Script.ttf", fontcolour, 28);
    SDL_QueryTexture(menutext, NULL, NULL, &W, &H);
    ...
    menutext = renderText("Select your grid size:", "assets/Brush Script.ttf", fontcolour, 30);
    SDL_QueryTexture(menutext, NULL, NULL, &W, &H);
